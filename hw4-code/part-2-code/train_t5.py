...
def get_device():
    """Get device with CUDA compatibility check"""
...
def get_args():
    ...
    # Pretrained model/tokenizer location (important for offline usage)
    parser.add_argument(
        '--pretrained_model_path',
        type=str,
        default='google-t5/t5-small',
        help=(
            "Either a Hugging Face model id (e.g. 'google-t5/t5-small') "
            "or a directory to be used as cache_dir for that model."
        ),
    )
    parser.add_argument(
        '--local_files_only',
        action='store_true',
        help="If set, load model/tokenizer only from local files (offline mode).",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Resume training from a specific epoch checkpoint (e.g., 5 to resume from model_epoch_5.pt)",
    )
...
def train(args, model, train_loader, dev_loader, optimizer, scheduler, tokenizer, start_epoch=0):
    best_f1 = -1
...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path, tokenizer):
    '''
    ...
    with torch.no_grad():
        for encoder_input, encoder_mask, decoder_input, decoder_targets, initial_decoder_input in tqdm(dev_loader):
            ...
            # Decode generated tokens to SQL strings
            for gen in generated:
                sql_query = tokenizer.decode(gen, skip_special_tokens=True)
...
def test_inference(args, model, test_loader, model_sql_path, model_record_path, tokenizer):
    '''
    ...
    with torch.no_grad():
        for encoder_input, encoder_mask, initial_decoder_input in tqdm(test_loader):
            ...
            # Generate SQL queries for test set
            generated = model.generate(
                input_ids=encoder_input,
                attention_mask=encoder_mask,
                decoder_start_token_id=initial_decoder_input[0, 0].item(),
...
def main():
    ...
    # Load tokenizer (can be local-only for offline compute nodes)
    # If pretrained_model_path is a directory, treat it as cache_dir for the HF model id.
    if os.path.isdir(args.pretrained_model_path):
        model_id = 'google-t5/t5-small'
        tokenizer = T5TokenizerFast.from_pretrained(
            model_id,
            cache_dir=args.pretrained_model_path,
    ...
    if args.resume_from_epoch is not None:
        ...
        model = load_model_from_checkpoint(args, epoch=args.resume_from_epoch)
        optimizer, scheduler = initialize_optimizer_and_scheduler(args, model, len(train_loader))
        # Note: When resuming, we start from the specified epoch, so the training loop will continue from there
    else:
        model = initialize_model(args)
        optimizer, scheduler = initialize_optimizer_and_scheduler(args, model, len(train_loader))

    # Train
    start_epoch = args.resume_from_epoch if args.resume_from_epoch is not None else 0
    train(args, model, train_loader, dev_loader, optimizer, scheduler, tokenizer, start_epoch=start_epoch)
    ...
    test_inference(args, model, test_loader, model_sql_path, model_record_path, tokenizer)
